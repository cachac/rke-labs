...
# Networking
resource "google_compute_address" "k3s_internal_address_worker03" {
  name         = "k3s-internal-address-worker23"
...
resource "google_compute_address" "k3s_external_address_worker03" {
  name   = "k3s-external-address-worker03"
  region = var.gcp_region
}

# disk: admin by google
resource "google_compute_disk" "k3s_worker_disk03" {
  name  = "k3s-worker-disk03"
...
resource "google_compute_instance" "k3s_worker03" {
  name         = "${var.prefix}worker03"
  ...
  boot_disk {
    source      = google_compute_disk.k3s_worker_disk03.id # "worker-disk-db03"
    auto_delete = false
  }
  ...
  network_interface {
    network    = "k3s-network"
    subnetwork = "k3s-subnet"
    network_ip = google_compute_address.k3s_internal_address_worker03.address

    access_config {
      nat_ip = google_compute_address.k3s_external_address_worker03.address
      # "35.238.114.204"
    }
  }
  ...
  metadata = {
    ...
    user-data = templatefile(
      ...
      # using script
      join("/", [path.module, "k3s_worker03_script.template"]),
      {
        docker_version = var.docker_version
        username       = local.node_username
        # node_internal_ip = google_compute_address.k3s_internal_address03.ad_workerdress
        node_public_ip = google_compute_address.k3s_external_address_worker03.address
      }
    )
  }
...
#   node_public_ip         = google_compute_instance.k3s_worker03.network_interface.0.access_config.0.nat_ip
#   node_internal_ip       = google_compute_instance.k3s_worker03.network_interface.0.network_ip
...
#   rancher_server_dns = join(".", ["rancher", google_compute_instance.k3s_worker03.network_interface.0.access_config.0.nat_ip, "xip.io"])
