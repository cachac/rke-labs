...
# Networking
resource "google_compute_address" "kube_internal_address_worker01" {
  name         = "kube-internal-address-worker01"
  subnetwork   = "kube-subnet"
  address_type = "INTERNAL"
  address      = "10.0.0.21"
  region       = var.gcp_region
}
# resource "google_compute_address" "kube_external_address_worker01" {
#   name   = "kube-external-address-worker01"
#   region = var.gcp_region
# }
...
resource "google_compute_instance" "kube_worker01" {
  ...
  network_interface {
    ...
    subnetwork = "kube-subnet"
    network_ip = google_compute_address.kube_internal_address_worker01.address
    # network_ip = "10.0.0.12"

		# worker hasn't an external IP
    # access_config {
    #   nat_ip = google_compute_address.kube_external_address_worker01.address
    # }
  }
  ...
  metadata = {
    ...
    user-data = templatefile(
      ...
      # using script
      join("/", [path.module, "kubeadm_worker01_script.template"]),
      {
        docker_version = var.docker_version
        username       = local.node_username
				node_private_ip = google_compute_address.kube_internal_address_worker01.address
      }
    )
  }
